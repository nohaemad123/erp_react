...
import { getAllSafes } from "@/services/loadData";
...
const initSearchValues = new SearchDto({
  selectColumns: ["id", "name", "balanceFirstDuration", "createdOn"],
});

interface IViewSafePageProps {
  tenantId: string;
}

export default function ViewSafePage({ tenantId }: Readonly<IViewSafePageProps>) {
  const { t, i18n } = useTranslation();
  const { isHttpClientLoading } = useAppStore();
  const [deletedPopup, setDeletedPopup] = useState<ISafe | null>(null);
  const handleClose = () => setDeletedPopup(null);
  const [rows, setRows] = useState<ISafe[]>([]);
  ...
  function fetchStores(params: SearchDto) {
    getAllSafes(i18n.language, tenantId, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleDelete(id: any) {
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.SAFE, {
        method: "DELETE",
        headers: {
          Tenant: tenantId,
  ...
  return (
    <div className="w-full">
      <div className="w-full p-8">
        <form className="w-full p-8 bg-white rounded-md" onSubmit={handleSubmit(handleSubmitForm)}>
          ...
          <div>
            ...
            <TextFieldAtom className="w-full" placeholder={t("Safe name")} {...register("search")} />
          </div>
        </form>

        <div className="flex items-center justify-between mt-8">
          <MainCardTitleAtom title="Safes" totalCount={pagination?.totalCount} />

          <div className="flex">
            <AddNewButtonAtom href={"/" + tenantId + "/safes/add"} />
          </div>
        </div>
        ...
        <TableAtom aria-label="Bank Card data">
          <TableAtom.THead>
            <TableAtom.TRow>
              ...
              <TableAtom.TCell component="th">{t("Balance first duration")}</TableAtom.TCell>
          ...
          <TableAtom.TBody>
            {rows.map((item, index) => (
              <TableAtom.TRow key={item?.id}>
                ...
                <TableAtom.TCellEnd
                  viewLink={"/" + tenantId + "/safes/" + item?.id}
                  editLink={"/" + tenantId + "/safes/" + item?.id + "/edit"}
      ...
      <ModalDeleteAtom
        ...
        titleMessage="Delete safe?"
        descriptionMessage="When you delete the safe, you will lose all the safe information and it will be transferred to the deleted list"
      />
    </div>
  );
}
