...
import { getAllPurchasesReturnInvoicesList, getPurchaseReturnInvoiceReport } from "@/services/loadData";
...
interface IPurchaseReturnInvoiceViewPageProps {
  tenantId: string;
}

export default function PurchaseReturnInvoiceViewPage({ tenantId }: Readonly<IPurchaseReturnInvoiceViewPageProps>) {
  const { t, i18n } = useTranslation();
  const { isHttpClientLoading } = useAppStore();
  const [rows, setRows] = useState<IPurchaseReturnInvoiceDto[]>([]);
  const [pagination, setPagination] = useState<IPagination>();
  const [deletedPurchaseReturnInvoice, setDeletedPurchaseReturnInvoice] = useState<IPurchaseReturnInvoiceDto | null>(null);
  const handleClose = () => setDeletedPurchaseReturnInvoice(null);
  ...
  useEffect(() => {
    fetchPurchaseReturnInvoices(initSearchValues);
  }, [tenantId]);

  const handleSubmitForm = (values: PurchasesSearchDto) => {
    values.page = 1;
    fetchPurchaseReturnInvoices(values);
    setValue("page", 1);
  };

  const handleReset = () => {
    reset({ ...initSearchValues, page: getValues("page"), pageSize: getValues("pageSize") });
    fetchPurchaseReturnInvoices({ ...initSearchValues, page: getValues("page"), pageSize: getValues("pageSize") });
  };

  function fetchPurchaseReturnInvoices(params: PurchasesSearchDto) {
    getAllPurchasesReturnInvoicesList(i18n.language, tenantId, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleDelete() {
    if (!deletedPurchaseReturnInvoice?.id) return;
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.PURCHASE_RETURN_INVOICE, {
        method: "DELETE",
        headers: {
          Tenant: tenantId,
        ...
        params: {
          id: deletedPurchaseReturnInvoice.id,
        },
      });

      if (response.status) {
        handleClose();
        fetchPurchaseReturnInvoices(getValues());
      } else {
  ...
  async function printHandler(id: string | null) {
    if (!id) return;
    try {
      const response = await getPurchaseReturnInvoiceReport(i18n.language, tenantId, id);
      if (!response) return;
  ...
  return (
    <div className="w-full">
        ...
        <div className="flex items-center justify-between mt-10">
          <div className="table_title_div px-6 pt-3 border-[#e6e7ec] bg-white border-t-2 border-l-2 border-r-2 rounded-ss-md rounded-se-md">
            <p className="pb-3 m-0 text-[12px] font-normal custom-border text-[var(--primary)]">
              {t("Purchase return invoice title")}
          ...
          <div className="flex">
            <AddNewButtonAtom href={"/" + tenantId + "/purchase-return-invoice/add"} />
          </div>
        </div>
                ...
                <TableAtom.TBody>
                  {rows.map((item, index) => (
                    <TableAtom.TRow key={item?.id}>
                      ...
                      <TableAtom.TCellEnd
                        viewLink={"/" + tenantId + "/purchase-return-invoice/" + item?.id}
                        editLink={"/" + tenantId + "/purchase-return-invoice/" + item?.id + "/edit"}
                        onDelete={() => setDeletedPurchaseReturnInvoice(item)}
                        onPrint={() => printHandler(item?.id ?? null)}
                      />
                    </TableAtom.TRow>
                  ))}
                </TableAtom.TBody>
      ...
      <ModalDeleteAtom
        isOpen={!!deletedPurchaseReturnInvoice}
        deleteHandler={handleDelete}
        closeHandler={handleClose}
        titleMessage="Delete purchase return invoice"
        descriptionMessage="When you delete the purchase return invoice, you will lose all the purchase return invoice information and it will be transferred to the deleted list"
      />
    </div>
  );
}
