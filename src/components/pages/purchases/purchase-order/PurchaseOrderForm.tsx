...
import { PurchaseOrderDetailsRow, PurchaseOrderDto } from "@/@types/dto/PurchaseOrderDto";
...
import { purchaseOrderValidationSchema } from "@/@types/validators/purchaseOrderValidators";
...
interface IPurchaseOrderFormTemplateProps {
  tenantId: string;
}

export default function PurchaseOrderForm({ tenantId }: Readonly<IPurchaseOrderFormTemplateProps>) {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<PurchaseOrderDto>({
    defaultValues: {
      ...new PurchaseOrderDto(),
      details: [new PurchaseOrderDetailsRow()],
    },
    resolver: valibotResolver(purchaseOrderValidationSchema),
  });
  ...
  const [filteredSupplier, setFilteredSupplier] = useState<ISupplier[]>([]);
  ...
  async function handleSubmitForm() {
    const orderData: PurchaseOrderDto = getValues();
    ...
    const ORDER_DATA = {
      ...
      details: orderData.details?.map((detailItem: PurchaseOrderDetailsRow, index: number) => {
  ...
  async function handleCreate(orderData: any) {
    try {
      const response = await fetchClient<ResultHandler<PurchaseOrderDto>>(EndPointsEnums.PURCHASE_ORDER, {
        method: "POST",
        body: orderData,
        headers: {
          Tenant: tenantId,
  ...
  function handleClearForm() {
    reset(new PurchaseOrderDto());
  }

  function handleAddRow() {
    const details: any = getValues("details");
    details.push(new PurchaseOrderDetailsRow());
  ...
  async function fetchNewCode() {
    if (!tenantId) return;
    const code = await getNewPurchaseOrderCode(i18n.language, tenantId);
  ...
  const calculateTotalTax = useCallback(() => {
    const totalRows = getValues("details");
    return totalRows.reduce((total, row) => total + (row.taxValue || 0), 0);
  }, [getValues]);
      ...
      <div className="w-full p-8 form_container">
        ...
        <div className="flex items-center justify-between">
          <div className="table_title_div px-8 pt-3 border-[#e6e7ec] bg-white border-t-2 border-l-2 border-r-2 rounded-ss-md rounded-se-md">
            <p className="pb-3 m-0 text-lg font-medium custom-border text-[var(--primary)]">{t("Purchase order details")}</p>
          </div>
        ...
        <div className="w-full p-10 bg-white border-b-2 table_container border-l-2 border-r-2 space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <Select disabled={isHttpClientLoading} value={value} onChange={onChange} placeholder={t("invoice type")} fullWidth>
            ...
            {/* supplier */}
            <div>
              <LabelAtom labelMessage="Supplier" />
              <Controller
                control={control}
                name="vendorId"
                render={({ field: { value, onChange } }) => (
                  ...
                  >
                    ...
                    >
                      <SearchDropMenuAtom
                        ...
                        onSearch={(value) => {
                          setFilteredSupplier(suppliersList.filter((item) => item.name?.toLowerCase().includes(value)));
                        }}
                      />
                    </MenuItem>
      ...
      <div className="w-full p-8 pt-0 form_container">
        <div className="w-full p-4 bg-white border-b-2 table_container border-l-2 border-r-2 mb-3">
          <TableAtom aria-label="product prices table">
            <TableAtom.THead>
              <TableAtom.TRow>
                <TableAtom.TCell align="center">#</TableAtom.TCell>
