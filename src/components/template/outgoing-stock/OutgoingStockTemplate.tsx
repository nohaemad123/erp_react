...
import {
  getNewOutgoingStoreCode,
  getAllOutgoingStoreTypes,
  getAllStores,
  getAllSuppliers,
  getAllProduct,
  getOutgoingStockTransactionById,
} from "@/services/loadData";
import { ButtonBase } from "@mui/material";
import { IOutgoingStockType } from "@/@types/interfaces/IOutgoingStockType";
import { OutgoingStockDto, OutgoingStockRow } from "@/@types/dto/OutgoingStockDto";
import dayjs from "dayjs";
import { OutgoingStockValidationSchema } from "@/@types/validators/OutgoingStockValidators";
...
import { IOutgoingStoreDetails } from "@/@types/interfaces/IOutgoingStoreTransactions";
...
interface IOutgoingStockFormTemplateProps {
  tenantId: string;
...
export default function OutgoingStockFormTemplate({
  tenantId,
  outgoingStockId,
  isEdit,
  isView,
}: Readonly<IOutgoingStockFormTemplateProps>) {
  ...
  const [OutgoingStockTypeList, setOutgoingStockTypeList] = useState<IOutgoingStockType[]>([]);
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<OutgoingStockDto>({
    defaultValues: {
      ...new OutgoingStockDto(),
      details: [new OutgoingStockRow()],
    },
    resolver: valibotResolver(OutgoingStockValidationSchema),
  });

  function handleAddRow() {
    const details: any = getValues("details");
    details.push(new OutgoingStockRow());
  ...
  async function handleSubmitForm() {
    const stockData: OutgoingStockDto = getValues();
    ...
    const STOCK_DATA = {
      ...
      date: stockData.date,
      note: stockData.note,
      outgoingStoreType: +stockData.outgoingStoreType,
      storeId: stockData.storeId,

      details: stockData.details?.map((detailItem: IOutgoingStoreDetails, index: number) => {
  ...
  async function handleCreate(stockData: any) {
    try {
      const response = await fetchClient<ResultHandler<OutgoingStockDto>>(EndPointsEnums.OUTGOING_STORE_TRANSACTION, {
        method: "POST",
        body: stockData,
        headers: {
          Tenant: tenantId,
  ...
  async function fetchNewCode() {
    if (!tenantId || outgoingStockId || isEdit || isView) return;
  ...
  useEffect(() => {
    if (outgoingStockId && typeof outgoingStockId === "string") {
      getOutgoingStockTransactionById(i18n.language, tenantId, outgoingStockId)
        .then((res) => {
          if (res) {
            const data = new OutgoingStockDto({
              ...res,
  ...
  useEffect(() => {
    if (tenantId) {
      getAllOutgoingStoreTypes(i18n.language, tenantId).then((res) => {
        setOutgoingStockTypeList(res ?? []);
      });
  ...
  const handleUnitChange = (row: OutgoingStockRow, unitId: string) => {
  ...
  const handleQtyChange = (row: OutgoingStockRow, qty: number) => {
    let cost = row.cost;
  ...
  const handlePriceChange = (row: OutgoingStockRow, price: number) => {
  ...
  const handleProductChange = (row: OutgoingStockRow, productId: string) => {
  ...
  const handleBarcodeChange = (row: OutgoingStockRow, barcode: string) => {
            ...
            {!isView && (
              <div className="flex gap-3 mb-2 btns-wrapper">
                ...
                {/* <Button
                  variant={"outlined"}
                   type="button"
                  onClick={() => push("/" + tenantId + "/outgoing-stock")}
          ...
          <div className="w-full p-10 bg-white border-b-2 table_container border-l-2 border-r-2 space-y-5 ">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
              ...
              <div>
                ...
                />
                <DropdownBoxAtom
                  options={OutgoingStockTypeList}
                  fullWidth
                  value={watch("outgoingStoreType")}
                  ...
                  onSelect={(item) => {
                    setValue("outgoingStoreType", item.key.toString());
                  }}
                />
                {errors.outgoingStoreType?.message && <ErrorInputAtom errorMessage={errors.outgoingStoreType?.message} />}
              </div>
