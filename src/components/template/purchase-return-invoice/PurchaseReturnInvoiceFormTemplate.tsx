...
import {
  PurchaseReturnInvoiceDetailsRow,
  IPurchaseReturnInvoiceDto,
  PurchaseReturnInvoiceDto,
} from "@/@types/dto/PurchaseReturnInvoiceDto";
...
import {
  GetAllInvoiceTypes,
  getAllProducts,
  getAllPurchasesInvoicesList,
  GetAllStores,
  getAllSuppliers,
  getAllTaxType,
  getDiscountType,
  getNewPurchaseReturnInvoiceCode,
...
import { purchaseReturnInvoiceValidationSchema } from "@/@types/validators/purchaseReturnInvoiceValidators";
...
interface IPurchaseReturnInvoiceFormTemplateProps {
  tenantId: string;
  invoiceId?: string;
  isEdit?: boolean;
  isView?: boolean;
...
export default function PurchaseReturnInvoiceFormTemplate({
  tenantId,
  invoiceId,
  isEdit,
  isView,
}: Readonly<IPurchaseReturnInvoiceFormTemplateProps>) {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<IPurchaseReturnInvoiceDto>({
    defaultValues: {
      ...new PurchaseReturnInvoiceDto(),
      details: [new PurchaseReturnInvoiceDetailsRow()],
    },
    resolver: valibotResolver(purchaseReturnInvoiceValidationSchema),
  });
  const [purchaseInvoiceList, setPurchaseInvoiceList] = useState<IPurchaseInvoiceDto[]>([]);
  ...
  useEffect(() => {
    getAllPurchasesInvoicesList(i18n.language, tenantId)
      .then((res) => {
        setPurchaseInvoiceList(res?.listData ?? []);
      })
      .catch(console.log);
  ...
  useEffect(() => {
    if (invoiceId && typeof invoiceId === "string") {
      getPurchaseReturnInvoiceById(i18n.language, tenantId, invoiceId)
        .then((res) => {
          if (res) {
            const data = new PurchaseReturnInvoiceDto({
              ...res,
              date: res.date ? dayjs(new Date(res.date)).toDate() : new Date(),
              details: res.details.map((detailItem: PurchaseReturnInvoiceDetailsRow) => {
                return {
                  ...detailItem,
  ...
  async function handleSubmitForm() {
    const invoiceData: IPurchaseReturnInvoiceDto = getValues();

    const INVOICE_DATA = {
      purchaseInvoiceHeadId: invoiceData.purchaseInvoiceHeadId,
      branchId: branch?.id ?? "",
      ...
      details: invoiceData.details?.map((detailItem: PurchaseReturnInvoiceDetailsRow, index: number) => {
  ...
  async function handleCreate(invoiceData: any) {
    try {
      const response = await fetchClient<ResultHandler<IPurchaseReturnInvoiceDto>>(EndPointsEnums.PURCHASE_RETURN_INVOICE, {
        method: "POST",
        body: invoiceData,
        headers: {
          Tenant: tenantId,
      ...
      if (response.status) {
        ...
        handleClearForm();
        push("/" + tenantId + "/purchase-return-invoice");
        return;
      }
    } catch (error: any) {
      console.log(error);
  ...
  function handleClearForm() {
    reset(new PurchaseReturnInvoiceDto());
  }

  function handleAddRow() {
    const details: any = getValues("details");
    details.push(new PurchaseReturnInvoiceDetailsRow());
  ...
  async function fetchNewCode() {
    if (!tenantId) return;
    const code = await getNewPurchaseReturnInvoiceCode(i18n.language, tenantId);
  ...
  const handleUnitChange = (row: PurchaseReturnInvoiceDetailsRow, unitId: string) => {
  ...
  const handleQtyChange = (row: PurchaseReturnInvoiceDetailsRow, qty: number) => {
    let cost = row.cost;
  ...
  const handleTaxChange = (row: PurchaseReturnInvoiceDetailsRow, taxId: string) => {
  ...
  const handleProductChange = (row: PurchaseReturnInvoiceDetailsRow, productId: string) => {
      ...
      <div className="w-full p-8 form_container">
        ...
        <div className="flex items-center justify-between">
          <div className="table_title_div px-8 pt-3 border-[#e6e7ec] bg-white border-t-2 border-l-2 border-r-2 rounded-ss-md rounded-se-md">
            <MainCardTitleAtom title="Purchase return invoice details" />
          </div>
          {!isView && (
        ...
        <div className="w-full p-10 bg-white border-b-2 table_container border-l-2 border-r-2 space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            {/* purchase invoices */}
            <div>
              <LabelAtom labelMessage="Purchase invoice number" required />
              <DropdownBoxAtom
                options={purchaseInvoiceList}
                fullWidth
                value={watch("purchaseInvoiceHeadId")}
                placeholder={t("Purchase invoice number")}
                filter={["code"]}
                optionRender={(item) => item.code}
                valueSelector={(item) => item.id}
                triggerLabelDisplay={(value) => value?.code ?? ""}
                onSelect={(item) => {
                  setValue("purchaseInvoiceHeadId", item?.id ? item.id : "");
                }}
              />
              {errors.purchaseInvoiceHeadId?.message && <ErrorInputAtom errorMessage={errors.purchaseInvoiceHeadId?.message} />}
            </div>
            ...
            <TableAtom.TBody>
              {watch("details")?.map((row, index) => (
                <TableAtom.TRow key={row.rowId}>
                  <TableAtom.TCell scope="row" align="center">
                    {index + 1}
                  </TableAtom.TCell>
                  <TableAtom.TCell>
                    <TextFieldAtom
          ...
          <div className="flex items-start justify-between mt-1">
            ...
            <div className="flex flex-col gap-4 p-3 border-solid border-2 border-gray-200">
              ...
              <div className="flex items-center justify-between text-basse total-item mt-2">
                ...
                <div className="value flex border-solid border-2 border-[#E2E8F0] rounded-md w-1/2">
                  <TextFieldAtom
                    disabled={isView || isHttpClientLoading}
                    // type="number"
                    size="small"
