...
import { SalesReturnInvoiceDetailsRow, SalesReturnInvoiceDto } from "@/@types/dto/SalesReturnInvoiceDto";
...
import {
  ...
  getAllSalesInvoicesList,
  GetAllStores,
  getAllTaxType,
  getDiscountType,
  getNewSalesReturnInvoiceCode,
...
import { salesReturnInvoiceValidationSchema } from "@/@types/validators/SalesReturnInvoiceValidators";
...
interface ISalesReturnInvoiceFormTemplateProps {
  tenantId: string;
  invoiceId?: string;
  isEdit?: boolean;
  isView?: boolean;
...
export default function SalesReturnInvoiceFormTemplate({
  tenantId,
  invoiceId,
  isEdit,
  isView,
}: Readonly<ISalesReturnInvoiceFormTemplateProps>) {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<SalesReturnInvoiceDto>({
    defaultValues: {
      ...new SalesReturnInvoiceDto(),
      details: [new SalesReturnInvoiceDetailsRow()],
    },
    resolver: valibotResolver(salesReturnInvoiceValidationSchema),
  });
  const [salesInvoicesList, setSalesInvoicesList] = useState<ISalesInvoiceDto[]>([]);
  ...
  useEffect(() => {
    getAllSalesInvoicesList(i18n.language, tenantId)
      .then((res) => {
        setSalesInvoicesList(res?.listData ?? []);
      })
      .catch(console.log);
  ...
  useEffect(() => {
    if (invoiceId && typeof invoiceId === "string") {
      getSalesReturnInvoiceById(i18n.language, tenantId, invoiceId)
        .then((res) => {
          if (res) {
            const data = new SalesReturnInvoiceDto({
              ...res,
              date: res.date ? dayjs(new Date(res.date)).toDate() : new Date(),
              details: res.details.map((detailItem: SalesReturnInvoiceDetailsRow) => {
                return {
                  ...detailItem,
  ...
  async function handleSubmitForm() {
    const invoiceData: SalesReturnInvoiceDto = getValues();
    ...
    const INVOICE_DATA = {
      ...
      salesInvoiceHeadId: invoiceData.salesInvoiceHeadId,
      ...
      details: invoiceData.details?.map((detailItem: SalesReturnInvoiceDetailsRow, index: number) => {
  ...
  async function handleCreate(invoiceData: any) {
    try {
      const response = await fetchClient<ResultHandler<SalesReturnInvoiceDto>>(EndPointsEnums.SALES_RETURN_INVOICE, {
        method: "POST",
        body: invoiceData,
        headers: {
          Tenant: tenantId,
      ...
      if (response.status) {
        ...
        handleClearForm();
        push("/" + tenantId + "/sales-return-invoice");
        return;
      }
    } catch (error: any) {
      console.log(error);
  ...
  function handleClearForm() {
    reset(new SalesReturnInvoiceDto());
  }

  function handleAddRow() {
    const details: any = getValues("details");
    details.push(new SalesReturnInvoiceDetailsRow());
  ...
  async function fetchNewCode() {
    if (!tenantId) return;
    const code = await getNewSalesReturnInvoiceCode(i18n.language, tenantId);
  ...
  const handleUnitChange = (row: SalesReturnInvoiceDetailsRow, unitId: string) => {
  ...
  const handleQtyChange = (row: SalesReturnInvoiceDetailsRow, qty: number) => {
    let cost = row.cost;
  ...
  const handleTaxChange = (row: SalesReturnInvoiceDetailsRow, taxId: string) => {
  ...
  const handleProductChange = (row: SalesReturnInvoiceDetailsRow, productId: string) => {
      ...
      <div className="w-full p-8 form_container">
        ...
        <div className="flex items-center justify-between">
          <div className="table_title_div px-8 pt-3 border-[#e6e7ec] bg-white border-t-2 border-l-2 border-r-2 rounded-ss-md rounded-se-md">
            <MainCardTitleAtom title="Sales return invoice details" />
          </div>
        ...
        <div className="w-full p-10 bg-white border-b-2 table_container border-l-2 border-r-2 space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            {/* <p>{ errors}</p> */}
            {/* <div>
              <LabelAtom labelMessage="Invoice code" />
              <TextFieldAtom
            ...
            {/* <div>
              ...
              <TextFieldAtom
                ...
                className="w-full h-12 px-5 py-1 mt-2 text-lg border-2 rounded-lg"
              />
              {errors.invoiceNumber?.message && <ErrorInputAtom errorMessage={errors.invoiceNumber?.message} />}
            </div> */}
            {/* sales invoice number */}
            <div>
              <LabelAtom labelMessage="Sales invoice number" required />
              <DropdownBoxAtom
                options={salesInvoicesList}
                fullWidth
                value={watch("salesInvoiceHeadId")}
                placeholder={t("Sales invoice number")}
                optionRender={(item) => item.code}
                valueSelector={(item) => item.id}
                triggerLabelDisplay={(value) => value?.id ?? ""}
                onSelect={(item) => {
                  setValue("salesInvoiceHeadId", item.id?.toString() || "");
                }}
              />
              {errors.salesInvoiceHeadId?.message && <ErrorInputAtom errorMessage={errors.salesInvoiceHeadId?.message} />}
            </div>
            {/* invoice type */}
            <div>
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <DatePickerInputFieldAtom
                    disabled={isView || isView || isHttpClientLoading}
                    value={dayjs(value)}
                    className="w-full"
            ...
            <TableAtom.TBody>
              {watch("details")?.map((row, index) => (
                <TableAtom.TRow key={row.rowId}>
                  <TableAtom.TCell component="th" align="center" scope="row">
                    {index + 1}
                  </TableAtom.TCell>
                  <TableAtom.TCell>
                    <TextFieldAtom
